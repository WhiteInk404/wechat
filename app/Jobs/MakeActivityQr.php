...
class MakeActivityQr implements ShouldQueue
{
    ...
    {
        $team_url = route('activity_team', ['activity_id' => $this->activity->id, 'team_id' => $this->team->id]);
        ...
        $image->insert(QrCode::format('png')->size(300)->margin(0)->generate($team_url), 'bottom-left', 170, 108)->save($path);
